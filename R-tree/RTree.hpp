...
public:
    ...
    void search(std::shared_ptr<Node>& cur_node,const Rectangle<N>& box,std::vector<ElemType>& result);
...
void RTree<N, ElemType, M, m>::insert(const Rectangle<N>& box,
    ...
    std::shared_ptr<Node> new_root(new Node);
    new_root->entry[0].box = root_pointer_->entry[0].box;
    new_root->entry[0].child_pointer = root_pointer_;
    new_root->entry[1].box = splitted_node->entry[0].box;
    new_root->entry[1].child_pointer = splitted_node;
    new_root->size = 2;
