...
//estructura quadtree para poder hacer el arbol e insertar el valor de cada pixel
struct quadtree{
    int valor;
    quadtree *nodo1;
    quadtree *nodo2;
...
//funcion para insertar el valor de cada pixel en el arbol
void insertar(quadtree *arbol, int valor){
    if(arbol->valor == -1){
        arbol->valor = valor;
    }else{
        if(valor < arbol->valor){
            if(arbol->nodo1 == NULL){
                arbol->nodo1 = new quadtree;
                arbol->nodo1->valor = -1;
                arbol->nodo1->nodo1 = NULL;
                arbol->nodo1->nodo2 = NULL;
                arbol->nodo1->nodo3 = NULL;
                arbol->nodo1->nodo4 = NULL;
            }
            insertar(arbol->nodo1, valor);
        }else{
            if(arbol->nodo2 == NULL){
                arbol->nodo2 = new quadtree;
                arbol->nodo2->valor = -1;
                arbol->nodo2->nodo1 = NULL;
                arbol->nodo2->nodo2 = NULL;
                arbol->nodo2->nodo3 = NULL;
                arbol->nodo2->nodo4 = NULL;
            }
            insertar(arbol->nodo2, valor);
        }
    }
}

//funcion para buscar el valor de cada pixel en el arbol
int buscar(quadtree *arbol, int valor){
    if(arbol->valor == valor){
        ...
        if(valor < arbol->valor){
            ...
            return buscar(arbol->nodo1, valor);
        }else{
            if(arbol->nodo2 == NULL){
                return 0;
            }
            return buscar(arbol->nodo2, valor);
        }
    }
}

//funcion para imprimir los pixeles de la imagen en una matriz
void imprimir(int **matriz, int32 width, int32 height){
...
int main()
{
  ...
  ReadImage("image.bmp", &pixels, &width, &height,&bytesPerPixel);
  int **matriz = new int*[height];
  for(int i = 0; i < height; i++){
      matriz[i] = new int[width];
  }
  for(int i = 0; i < height; i++){
      for(int j = 0; j < width; j++){
          matriz[i][j] = pixels[i*width + j];
      }
  }
  imprimir(matriz, height, width);
}